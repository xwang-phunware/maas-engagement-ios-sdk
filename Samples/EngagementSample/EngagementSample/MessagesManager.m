...
- (void)refreshBadgeCounter {
    dispatch_async(dispatch_get_main_queue(), ^{
        NSArray *messages = [PWEngagement messages];
        NSInteger unreadMessageCount = [[self getUnreadMessagesFrom:messages] count];
        // Set app icon
        if ([UIApplication sharedApplication].applicationIconBadgeNumber != unreadMessageCount) {
...
- (NSArray*)getReadMessagesFrom:(NSArray*)messages {
    ...
    for (PWMEZoneMessage *message in messages) {
        if (message.isRead) {
