...
- (void)fetchProfileAttrbuteMetadata {
    ...
    [[PWMEAttributeManager sharedManager] fetchProfileAttributeMetadataWithCompletion:^(NSArray *attributes, NSError *error) {
        if (error) {
...
- (void)fetchProfileAttributesForDevice {
    [[PWMEAttributeManager sharedManager] fetchProfileAttributesWithCompletion:^(NSDictionary *attributes, NSError *error) {
        if (error) {
            [PubUtils displayError:error];
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (IBAction)saveProfileAttribute:(id)sender {
    // Save them
    [PubUtils showLoading];
    for (NSString *key in [self.userAttributes allKeys]) {
        if ([self.userAttributes[key] length] == 0) {
            [self.userAttributes removeObjectForKey:key];
        }
    }
    
    [[PWMEAttributeManager sharedManager] updateProfileAttributes:self.userAttributes completion:^(NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (error) {
                [PubUtils displayError:error];
            } else {
